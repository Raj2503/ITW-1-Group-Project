...
def percentage():
    ...
    if clicked.get()=='Course6':
        x=5
    for i in range(5):
        name = Label(frame,text=dflist[x]['Name'][i]).grid(row=i+1,column=0)
        attended = (dflist[x].iloc[i,4:]=="Present").sum()
        total = 40 - (dflist[x].iloc[1,4:].isnull()).sum()
...
def open():
    ...
    for i in range(5):
        ...
        Present.append(c)
        Absent.append(d)
    btn2 = Button(frame,text="Close Window",command=lambda:[classroom.withdraw(),root.deiconify(),save()]).grid(row=11,column=0,ipadx=20,pady=30)
